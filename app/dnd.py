...
# 职业 → 技能池：技能选择种类与职业绑定（法师偏法术、战士偏近身、盗贼偏潜行…）
CLASS_SKILLS = {
    "战士": ["运动", "调查", "洞察", "历史"],
    "野蛮人": ["运动", "威吓", "生存", "自然"],
    "武僧": ["运动", "隐匿", "洞察", "说服"],
    "盗贼": ["隐匿", "手上功夫", "开锁", "欺瞒", "侦查", "说服"],
    "游侠": ["生存", "侦查", "自然", "动物驯化", "医疗"],
    "德鲁伊": ["生存", "自然", "动物驯化", "医疗", "洞察"],
    "牧师": ["宗教", "医疗", "洞察", "说服"],
    "圣武士": ["运动", "威吓", "宗教", "洞察"],
    "吟游诗人": ["表演", "欺瞒", "说服", "威吓", "调查"],
    "法师": ["奥秘", "调查", "历史", "说服"],
...
# 职业 → 法术池(亲和)：施法者可选法术按职业限定种类
CLASS_SPELLS = {
    "法师": ["神导术", "火钳术", "戏法·法师之手", "戏法·光亮术", "戏法·舞蹈术", "燃烧之手", "魔法飞弹", "法师护甲", "睡眠术", "侦测魔法", "护盾术", "火球术", "闪电束", "飞行术"],
    "术士": ["神导术", "火钳术", "戏法·光亮术", "魔法飞弹", "燃烧之手", "睡眠术", "护盾术", "火球术", "闪电束", "飞行术"],
    "邪术师": ["火钳术", "戏法·舞蹈术", "燃烧之手", "不谐低语", "护盾术", "火球术", "放逐术", "飞行术"],
    "吟游诗人": ["戏法·舞蹈术", "戏法·光亮术", "不谐低语", "睡眠术", "魔法飞弹", "治愈伤处", "飞行术", "群体治愈"],
    "牧师": ["神导术", "圣火术", "祝福术", "命令术", "治愈伤处", "群体治愈", "放逐术"],
    "圣武士": ["命令术", "祝福术", "治愈伤处", "圣火术"],
    "德鲁伊": ["神导术", "橡木之心", "治愈伤处", "群体治愈"],
    "游侠": ["橡木之心", "治愈伤处"],
}

# 职业 → 主武器与伤害骰（命中检定与伤害结算用它）
CLASS_WEAPON = {
    "战士": ("长剑", 8, "力量"),
    "野蛮人": ("巨斧", 12, "力量"),
    "武僧": ("短棒", 6, "敏捷"),
...
def class_skill_pool(klass: str) -> list[str]:
    """职业相关的可选熟练技能池（战士偏近身、盗贼偏潜行、法师偏法术…）。"""
    return list(CLASS_SKILLS.get(klass, [])) or [n for n in SKILL_ABILITY if n not in CLASS_SKILLS.get("战士", [])]


def class_spell_pool(klass: str) -> list[str]:
    """职业法术亲和池：施法者可选法术限定在职业池内。"""
    return list(CLASS_SPELLS.get(klass, []))


def background_items(background: str) -> list[dict]:
    """身份（背景）自带装备：创建时自动分配。"""
    name, value = BACKGROUND_ITEMS.get(background, (None, 0))
    if not name:
        return []
    return [{"name": name, "desc": "身份装备", "effect": "", "qty": 1, "value": value}]


def class_weapon(klass: str) -> tuple[str, int, str]:
    """职业主武器：(武器名, 伤害骰面数, 命中加值能力)。"""
    return CLASS_WEAPON.get(klass, ("简陋木棍", 4, "力量"))


def capability_cap(level: int) -> int:
    """法术/技能选择上限：初始 2 个，之后每升一级增加 1 个。"""
    return max(1, 2 + (max(1, level) - 1))
